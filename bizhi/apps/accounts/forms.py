...
class RegisterForm(forms.ModelForm):
    ...
    mobile_captcha = forms.CharField(label="验证码", widget=widgets.TextInput(attrs={"style":"width: 160px;padding: 10px", "placeholder":"请输入验证码", "error_messages": {"invalid": "验证码错误"}}))
    class Meta:
        model = User
        ...
        widgets = {
            ...
            'email': widgets.TextInput(attrs={"class": "form-control", "placeholder": "请输入邮箱"}),
        }
    ...
    def clean_email(self):
        ret1 = User.objects.filter(email=self.cleaned_data.get("email"))
        if not ret1:
